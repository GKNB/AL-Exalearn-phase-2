...
for epoch in range(0, args.epochs):
    ...
    if epoch > args.blind_train_epoch:
        ...
        if early_stopping.improve:
            ...
            #Here in the first iteration we save two copy of the model, ckpt is for AL branch, ckpt_base is for baseline (no AL) branch
...
def validation():
    y_pred_torch_class, y_pred_torch_regression = model(x_test_torch)
    y_pred_torch_class = y_pred_torch_class.detach()
    y_pred_torch_regression = y_pred_torch_regression.detach().reshape(-1, 4)
